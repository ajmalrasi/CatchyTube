...
//        dbHelper.close();
     ...
     *          inserted task id
     */
    public int addTask(String videoId,String saveName, String url,String aUrl, int chunk,
                       String sdCardFolderAddress, boolean overwrite,
                       boolean priority){
        Log.w("DLDManagerPro Threads",Thread.currentThread().getName());
        ...
        if (aUrl!=null){
            return insertNewTask(videoId,saveName, url,aUrl, chunk, sdCardFolderAddress, priority);
        }
        return insertNewTask(videoId,saveName, url, chunk, sdCardFolderAddress, priority);
    }


    public int addTask(String videoId,String saveName, String url, int chunk, boolean overwrite, boolean priority){
        return this.addTask(videoId,saveName, url,AUDIO_URL, chunk, SAVE_FILE_FOLDER, overwrite, priority);
    }

    public int addTask(String videoId,String saveName, String url, boolean overwrite, boolean priority) {
        return this.addTask(videoId,saveName, url,AUDIO_URL, maximumUserChunks, SAVE_FILE_FOLDER, overwrite, priority);
    }

    public int addTask(String videoId,String saveName, String vUrl,String aUrl, boolean overwrite, boolean priority) {
        return this.addTask(videoId,saveName, vUrl,aUrl, maximumUserChunks, SAVE_FILE_FOLDER, overwrite, priority);
    }
    ...
    public void pauseAudioDownload(int token) {
        moderator.audioPause(token);
    }
    ...
    public List singleChunkStatus(int token) {
        Task task = tasksDataSource.getTaskInfo(token);
        if (task != null) {
            List<Chunk> taskChunks = chunksDataSource.chunksRelatedTask(task.id);
            return taskChunks;
    ...
    public boolean delete(int token, boolean deleteTaskFile){
        ...
        if (task.url != null){
            ...
            if (deleteTaskFile) {
                //delete mux file instead if dash
    ...
    private int insertNewTask(String videoId,String taskName, String url, int chunk, String save_address, boolean priority) {
        Task task = new Task(videoId,0, taskName, url, TaskStates.INIT, chunk, save_address, priority);
        task.id = (int) tasksDataSource.insertTask(task);
        Log.d("--------", "task id "+String.valueOf(task.id));
        //-----------------notify----------------
        downloadManagerListener.OnDownloadInit(task.id);
        return task.id;
    }
    //dash download
    private int insertNewTask(String videoId,String taskName, String vUrl,String aUrl, int chunk, String save_address, boolean priority) {

        Task task = new Task(videoId,0, taskName, vUrl,aUrl, TaskStates.INIT, chunk, save_address, priority);
        task.id = (int) tasksDataSource.insertTask(task);
        Log.d("--------", "dash download init task id "+String.valueOf(task.id));
        //-----------------notify----------------
        downloadManagerListener.OnDownloadInit(task.id);
        return task.id;
    }
    ...
    private String getUniqueName(String name){
        ...
        ext = uniqueName.substring(uniqueName.lastIndexOf("."));
        uniqueName = uniqueName.replace(ext,"");
        format = uniqueName.substring(uniqueName.lastIndexOf("-"));
        uniqueName = uniqueName.replace(format,"");
        name = uniqueName;
        int count = 1;
        ...
        Log.v("PRO",Thread.currentThread().getName());
        uniqueName = uniqueName.concat(format).concat(ext);
        return uniqueName;
    }
