...
import catchytube.com.rasi.jsinterpreter.JsEvaluator;
import catchytube.com.rasi.interpreterinterface.JsCallback;
...
public abstract class YouTubeExtractor extends AsyncTask<String, Void, SparseArray<YtFile>> {
    ...
    static {
        ...
        FORMAT_MAP.put(313, new Format(313, "webm", 2160, Format.VCodec.VP9, Format.ACodec.NONE, true));//4k
        FORMAT_MAP.put(272, new Format(272, "webm", 4320, Format.VCodec.VP9, Format.ACodec.NONE, true));//8k
...
//        FORMAT_MAP.put(250, new Format(250, "webm", Format.VCodec.NONE, Format.ACodec.OPUS, 64, true));
    ...
    private SparseArray<YtFile> getStreamUrls() throws IOException, InterruptedException {
        ...
        try {
            ...
            //
            //writeToFile(streamMap,"extractor.log");
    ...
    public void writeToFile(String log,String filename) {
        File path = new File(getExternalStorageDirectory()+ "/arscast/");
        String logfile =log;
        ...
        try {
            ...
            for(int i=0;i<5;i++){
                logfile = URLDecoder.decode(logfile,"UTF-8");
            }
            myOutWriter.append(logfile);
            myOutWriter.close();
            fOut.flush();
            fOut.close();
        } catch (IOException e) {
            Log.e("Exception", "Log File write failed: " + e.getMessage());
        }
    }
